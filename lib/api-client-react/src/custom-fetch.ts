...
// Проверяем, является ли ошибка сигналом режима обслуживания
function isMaintenanceModeError(data: unknown): boolean {
  if (!data || typeof data !== "object") return false;
  const errorData = data as { code?: string; error?: string };
  return (
    errorData.code === "MAINTENANCE_MODE" ||
    (typeof errorData.error === "string" &&
      errorData.error.includes("технического обслуживания"))
  );
}
