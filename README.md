<p align="center"><img src="artifacts/readora/public/og-image.png" alt="Readora"></p>
...
- **Загрузка книг FB2 и EPUB** с автоматическим извлечением метаданных, обложки и оглавления
...
### Миграции в CI и CapRover

- GitHub Actions поднимает чистый PostgreSQL 16, применяет все миграции и повторяет запуск, чтобы проверить идемпотентность migration runner до сборки образа.
- Production-контейнер CapRover запускает `migrate-unified.mjs apply` до запуска Express. Нужен только обычный `DATABASE_URL` приложения; отдельный доступ CI к production БД не требуется.
- Запуск защищён PostgreSQL advisory lock: несколько одновременно стартующих контейнеров не выполнят новую миграцию параллельно. Уже зарегистрированные миграции пропускаются по checksum.
- Если новая миграция завершится ошибкой, новый контейнер не запустит приложение. Для изменений, несовместимых со старой версией, используйте expand/contract-подход и несколько релизов.
...
## 🛠 Управление через readora-manager.sh

Скрипт `readora-manager.sh` — единая точка входа для разработки и деплоя Readora. Он объединяет все рутинные операции (установка зависимостей, настройка окружения, запуск сервисов) в простые команды.

### Команды

| Команда | Описание |
|---------|----------|
| `setup` | Полная установка на новом сервере: проверка зависимостей, создание `.env`, запуск PostgreSQL, миграции, создание супер-администратора |
| `start` | Запуск в режиме разработки с hot-reload (frontend + backend) |
| `stop` | Остановка всех Docker-сервисов и локальных процессов |
| `restart` | Перезапуск приложения |
| `status` | Показать статус всех сервисов и портов |
| `services` | Запустить только PostgreSQL |
| `db` | Применить миграции базы данных |
| `build` | Сборка production-версии |
| `start:prod` | Информация о запуске production-режима (требует настройки process manager) |
| `check` | Проверка типов TypeScript |
| `logs` | Просмотр логов менеджера |
| `clean` | Очистка логов менеджера |
| `help` | Показать справку |

### Примеры использования

```bash
# Полная установка на новом сервере (интерактивная)
./readora-manager.sh setup

# Запуск в режиме разработки (как раньше)
./readora-manager.sh start

# Проверка статуса сервисов
./readora-manager.sh status

# Пересборка проекта
./readora-manager.sh build
```

> **Примечание:** Стандартные pnpm-команды из README также продолжают работать — используйте их, если предпочитаете ручной контроль. Скрипт `readora-manager.sh` — это удобная обёртка, а не замена.
...
- **Квоты библиотеки** — администратор задаёт максимальный размер одной книги и общий логический объём книг для обычного пользователя; лимиты не применяются к администраторам
...
- Контакт для заинтересованных лиц: [svv@8ls.ru](mailto:svv@8ls.ru)
